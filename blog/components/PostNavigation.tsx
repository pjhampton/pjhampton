      ...
      <div className="w-1/2 pr-4">
        {previousPost && (
          <Link href={`/post/${previousPost.slug}`}>
            <div className={`group cursor-pointer p-4 rounded-lg transition-all duration-200 ${
              resolvedTheme === 'dark' 
      ...
      <div className="w-1/2 pl-4">
        {nextPost && (
          <Link href={`/post/${nextPost.slug}`}>
            <div className={`group cursor-pointer p-4 rounded-lg transition-all duration-200 text-right ${
              resolvedTheme === 'dark' 
                ? 'bg-gray-800/50 hover:bg-gray-800' 
                : 'bg-[#f9f9f9] hover:bg-gray-200'
            }`}>
